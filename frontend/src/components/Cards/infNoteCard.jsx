  ...
  return (
    <div className={`border ${viewMode === 'list' ? 'border-slate-300':''} rounded-lg bg-white hover:shadow-xl transition-all ease-in-out flex items-center justify-between`}>
      <div className={`rounded-lg px-4 py-2 w-[90%] ${viewMode === 'list' ? 'flex items-center gap-6 justify-between' : ''}`} onClick = {onView}>
           ...
           <h6
            ...
            <div className="flex flex-col">
              <span className="text-xs text-green-700">
                Создано: {moment(date, "YYYY-MM-DDTHH:mm:ss.SSSZ").format("Do MMM YYYY")}
              </span>
              <span className="text-xs text-blue-700">
                Обновлено: {moment(upDate, "YYYY-MM-DDTHH:mm:ss.SSSZ").format("Do MMM YYYY")}
              </span>
            </div>
          </div>
        </div>
        ...
        <p
          className="text-xs text-slate-600 mt-2 break-words overflow-hidden max-w-full truncate"
          title={description}
        >
